...
export const roomsSlice = createSlice({
	...
	reducers: {
		...
		updateMessageLasted: (state, action) => {
			const rooms = state.rooms.map((object: any) =>
				object.roomId === action.payload.roomId
					? { ...object, messages: { ...action.payload } }
					: { ...object }
			);
			state.rooms = [...rooms];
		},
...
export const { setSkip, updateMessageLasted } = roomsSlice.actions;
